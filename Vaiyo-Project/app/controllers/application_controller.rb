...
class ApplicationController < ActionController::Base
  ...
  def room_total_recordings(bbb_id)
    return recordings(bbb_id).count
  end
  helper_method :room_total_recordings
  ...
  def all_participates(room_id)
    all_users = []
    users = SharedAccess.where(:room_id => room_id)

    if users
      users.each do |user|
        all_users.push User.find_by_id(user[:user_id])
      end
    end

    return all_users
  end
  helper_method :all_participates

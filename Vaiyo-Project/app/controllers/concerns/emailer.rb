...
module Emailer
  ...
  # Sends password reset email.
  def acc_info_verification(user, token, user_new_email)
    begin
      # return unless Rails.configuration.enable_email_verification
      UserMailer.acc_info_verification(user, token, user_new_email, @settings).deliver_now
    rescue => e
      logger.error "Support: Error in email delivery: #{e}"
      flash[:alert] = I18n.t(params[:message], default: I18n.t("delivery_error"))
    else
      flash[:success] = I18n.t("email_sent", email_type: t("mailer.user.verify_acc_info.alert_subtitle"))
    end
  end

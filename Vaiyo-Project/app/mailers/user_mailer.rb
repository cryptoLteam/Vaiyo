...
class UserMailer < ApplicationMailer
  ...
  def acc_info_verification(user, token, user_new_email, settings)
    @settings = settings
    @user = user
    @verification_link = ENV['SAFE_HOSTS'] + '/verify-account-info/' + token
    @image = logo_image
    @color = user_color

    # raise RuntimeError, @settings.to_yaml

    mail to: user_new_email, subject: t('mailer.user.verify_acc_info.title')
  end
end
